...
def lambda_handler(event, context):
    ...
    logger.info("Pull Terraform state")
    with open('/tmp/terraform.tfstate', 'w') as f:
        pull_result = subprocess.run(["terraform", "state", "pull"], cwd="/tmp", stdout=f, stderr=subprocess.PIPE, text=True)
    logger.info(f"Terraform pull stderr: {pull_result.stderr}")
    ...
    logger.info("Destroy Terraform configuration")
    destroy_result = subprocess.run(["terraform", "destroy", "-auto-approve"], cwd="/tmp", capture_output=True, text=True)
    logger.info(f"Terraform destroy output: {destroy_result.stdout}\n{destroy_result.stderr}")

    return {
        'statusCode': 200,
        'body': destroy_result.stdout + destroy_result.stderr  
    }
