##############################################################
# Création de l'instance EC2 node21 t3.medium avec OS ubuntu #
############################################################## 
resource "aws_instance" "node21" {
...
# Définir un nouveau nom d'hôte pour node21
...
# après avoir configuré la pub_ssh_key sur controlplane2
systemctl restart sshd

# Récupérer l'IP privée du node21
private_ip=$(curl -s http://169.254.169.254/latest/meta-data/local-ipv4)
# Stocker l'IP dans un fichier S3
echo "$private_ip" > /tmp/node21-ip.txt
aws s3 cp /tmp/node21-ip.txt s3://kubeadv786/node21-ip.txt

#Boucle d'enregistrement du node21 dans le cluster ainsi que mise en place sshd correct et /etc/hosts
max_attempts=10
attempt=0
while [ $attempt -lt $max_attempts ]; do
  echo "Tentative $((attempt+1)) sur $max_attempts"
  # Télécharger le fichier d'enregistrement
  if aws s3 cp s3://kubeadv786/node21-registration.txt /tmp/node21-registration.txt; then
    # Exécuter la commande d'enregistrement
    if $(cat /tmp/node21-registration.txt); then
      # Récupérer la clé publique
      if aws s3 cp s3://kubeadv786/controlplane2-authorized-key.txt /root/.ssh/authorized_keys; then
        # Récupérer le fichier config k8s
        if aws s3 cp s3://kubeadv786/node21-config.txt /root/.kube/config; then
          # Mettre à jour le fichier hosts
          if controlplane2_ip=$(aws s3 cp s3://kubeadv786/controlplane2-ip.txt - | tr -d '\r') && [ -n "$controlplane2_ip" ]; then
            echo "$controlplane2_ip  controlplane2" >> /etc/hosts
...
exit 1
  ...
  depends_on = [aws_instance.controlplane2]
