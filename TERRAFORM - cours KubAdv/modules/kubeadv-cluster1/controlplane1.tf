...
controlplane1_authorized_key=$(cat /root/.ssh/id_rsa.pub)
# Stocker la clé publique dans un fichier S3
echo "$controlplane1_authorized_key" > /tmp/controlplane1-authorized-key.txt
aws s3 cp /tmp/controlplane1-authorized-key.txt s3://kubeadv786/controlplane1-authorized-key.txt
